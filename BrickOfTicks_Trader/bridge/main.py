...
class BridgeEngine:
    ...
    def _check_be(self, tick):
        if self.risk.check_be_trigger(tick, self.state):
            ...
            logger.info(f"Triggering Break-Even for ticket {ticket} to SL: {entry_price}")
            ...
            if conf and conf['status'] == 'OK':
                ...
                logger.info(f"BREAK-EVEN success: SL moved to {entry_price:.5f}")
            else:
                logger.error(f"BREAK-EVEN failed to execute on MT5 for ticket {ticket}.")
