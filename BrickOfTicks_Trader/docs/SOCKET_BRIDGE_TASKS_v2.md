...
### 3.1 Implementation
- [x] Create `bridge/renko.py` — port from `bot_implementation.md` Phase 2
- [x] **[UPDATED]** Set `K_MULTIPLIER = 0.00295` as module-level constant (not 0.0018, not 0.00118)
- [x] Add deprecation comment: `# DEPRECATED: 0.00118 (unprofitable), 0.0018 (old bot spec)`
- [x] `BrickEvent` namedtuple: `(open, close, high, low, uptrend, timestamp, brick_size, sequence)`
- [x] `RenkoBuilder.__init__(day_open_price)`: compute `self.brick_size = day_open_price * K_MULTIPLIER`
- [x] `update_tick(bid_price, time_ms)`: UP/DOWN while-loop with 2× reversal rule, gap-fills
- [x] `update_brick_size(new_day_open)`: update `self.brick_size = new_day_open * K_MULTIPLIER`
- [x] Sequence tracking: binary string, `maxlen=100`

### 3.2 Verification
- [x] Assert `K_MULTIPLIER == 0.00295` in unit test
- [x] Example test: `day_open=2400`, `brick_size` should be `7.08 ±0.001`
- [x] Manual test: UP bricks at 2407.08, 2414.16; reversal requires reaching 2400.00 (2× below 2414.16)
- [x] Gap-fill test: price jumps from 2400 to 2435.4 → exactly 5 bricks emitted (5 × 7.08)
- [x] Extreme volatility test: 50 ticks arrive with large jumps → assert gap-fill bricks sequence has contiguous open/close levels
- [x] **[UPDATED]** Parity test: feed bid prices from `Data/Processed/XAUUSD_Holdout_K00295.csv` → brick count and open/close prices match CSV (not the old K=0.00118 CSV)
...
### 4.1 Implementation
- [x] Copy `src/feature_engine.py` → `bridge/feature_engine.py`
- [x] Verify `LiveFeatureEngine` class (not batch version) is present
- [x] Verify `RollingZScore`: window=1000, warmup=30, Welford O(1)
- [x] **[UPDATED]** Verify volume fallback is fully implemented:
  - `if bid_vol <= 0 or ask_vol <= 0:` → `raw_ofi = sign(Δmid)`, `depth_raw = 0.0`, `susc_raw = 0.0`
- [x] Verify susceptibility: `susc_raw = raw_ofi / (depth_raw + 1e-8)` BEFORE z-score
- [x] Verify OFI weak inequalities: `dBid >= 0` and `dBid <= 0` (not strict)

### 4.2 Verification
- [x] Parity test: load 200 ticks from `outputs/holdout_K295/features/` → max feature error < 1e-4
- [x] First tick: assert returns `[0.0] * 9`
- [x] **[NEW]** Volume fallback test: inject tick with `bid_vol=0, ask_vol=0`, price up → assert `raw_ofi == 1.0`
- [x] Susceptibility guard: tick with `depth_raw=0` → no crash, no NaN
- [x] Weak inequality test: tick where `dBid == 0` (price unchanged, volume changes) → OFI ≠ 0 (volume refresh captured)
- [x] Z-score sliding window test: push 1500 ticks, assert incremental mean/var exactly matches `np.mean()` and `np.var()` of the last 1000 ticks
...
### 5.1 Buffer Port
- [x] Copy `inference/buffer.py` → `bridge/buffer.py`
- [x] Assert `micro_buffer = deque(maxlen=100)` — NOT 5000 (Phase 1 uses 100 for model compatibility)
- [x] Assert `on_brick_close()` rewrites Flag_Curr (idx 6) and Decay (idx 8)
- [x] Assert zero-padding at front when < 100 ticks
- [x] Assert returns `None` when < 10 bricks in history

### 5.2 Ensemble Port
- [x] Copy `inference/ensemble.py` → `bridge/ensemble.py`
- [x] **[UPDATED]** Set `PRED_OS_THRESHOLD = 1.4` (unified for K=0.00295)
...
- [x] **[UPDATED — BAITING REMOVED]** Remove all code paths that return `action=-1`
  - Delete `is_bait` calculation
  - Delete baiting threshold constants
  - Simplify return: `action = 1 if votes >= VOTE_THRESHOLD else 0`
- [x] Model paths: primary `BrickOfTicks_Trader/models/fold_N/model.keras`, fallback `outputs/exec/cv/fold_N/model.keras`
- [x] On startup: if any model fails to load → log CRITICAL + `sys.exit(1)`

### 5.3 Verification
- [x] Assert `PRED_OS_THRESHOLD == 1.4` in unit test
- [x] **[UPDATED]** Confirm threshold produces correct WR: load `outputs/tensors_holdout_K295/holdout_*.npy` → assert WR at threshold 1.4 is within 5% of 90.3%
- [x] **[NEW]** Baiting disabled: assert no code path in `predict()` ever returns `action=-1`
  - Test: mock 3 models with `prob_win=0.05, pred_os=0.3` → assert `action == 0` (SKIP, not -1)
- [x] Voting: 3/3=ENTER, 2/3=ENTER, 1/3=SKIP, 0/3=SKIP
...
### 6.1 State Manager
- [x] Create `bridge/state.py`
- [x] Implement `DEFAULT_STATE` with all 12 fields (see IMPL_v2 Phase 6)
- [x] `load()`: read `bridge/logs/state.json`, merge with DEFAULT_STATE (supports new fields)
- [x] `save()`: atomic write (write to `.tmp` file, then `os.rename()`) to prevent corruption on crash
- [x] `update(key, value)`: update field + call `save()` immediately
- [x] `get(key)`: return field value

### 6.2 Risk Manager
- [x] Create `bridge/risk.py`
- [x] `check_daily_limit(daily_pnl_pts, brick_size)`: return `False` if `daily_pnl_pts < -(5 × brick_size)`
  - Rationale: 5 losses at 1× brick_size = 5 SL hits (conservative daily halt)
- [x] `check_position_open(state)`: return `False` if `state.get('active_ticket') != 0`
- [x] **[UPDATED]** `check_be_trigger(tick, state)`: `True` if price moved `>= 0.3125 × active_brick_size` favorably
...
### 6.3 Trade Logger
- [x] Create `bridge/trade_logger.py`
- [x] CSV columns: `timestamp, brick_dir, fold1_pw, fold1_os, fold2_pw, fold2_os, fold3_pw, fold3_os, votes, action, entry, sl, tp, ticket, outcome, pnl_pts, entry_spread_pts`
- [x] `log_signal(brick, decision)`: write inference row (outcome=PENDING initially)
- [x] `log_order(ticket, entry, sl, tp, direction)`: update row with order details
- [x] `log_outcome(ticket, outcome, pnl_pts)`: update row when trade closes
- [x] `generate_session_report()`: compute WR, expectancy, trade count; compare to backtest baseline (90.3% WR); save `.md` summary

### 6.4 Verification
- [x] StateManager: write state → kill process → reload → all 14 fields recovered correctly
- [x] Atomic save test: truncate `.tmp` during write → original JSON intact
- [x] Risk limit: `daily_pnl=-40, brick_size=7.08` → `check_daily_limit()` returns `False` (−40 < −35.4)
- [x] BE trigger: `entry=2400, brick_size=7.08, direction=1, bid=2402.22` → `0.3125×7.08=2.2125 → True`
- [x] Trade logger: 10 signals → CSV has 10 rows, all columns populated
- [x] Logging format test: verify trades.csv header matches exact columns and `entry_spread_pts` is recorded correctly
...
- [x] `BridgeEngine.__init__()`: initialize all components
- [x] `start()`: start receiver → load models → connect sender → wait DAYOPEN (30s timeout)
- [x] `_warmup()`: wait `history_done` event (30s timeout) → replay all `history_ticks` via `_process_tick(..., is_warmup=True)` → log result
- [x] Warmup gate: `brick_count >= 10` AND `len(z_ofi.deque) >= 1000` → set `state.warmup_done = True`
- [x] Warmup Live Mode fallback:
...
- [x] `_run()`: `tick_queue.get(timeout=60)` loop; if `Empty` → log WARNING + check degraded mode
- [x] `_process_tick()`: feature_engine → buffer.append → renko.update → for each new brick: `buffer.on_brick_close()` → if tensors + not warmup → `_on_signal()`
- [x] `_on_signal()`: risk checks → ensemble.predict → if action==1 → compute SL/TP → send buy/sell → update state
- [x] `_check_be()`: on every tick, check if break-even trigger met → send MODIFYSL command

### 7.2 **[NEW]** Rollover Implementation
- [x] **[UPDATED]** Split rollover into 3 sub-tasks:
...
- [x] Open position during rollover: verify state keeps `active_sl`, `active_tp`, `active_brick_size` unchanged

### 7.3 **[NEW]** Degraded Mode Implementation
- [x] `_enter_degraded_mode()`: set `state.degraded_mode = True`; log CRITICAL; suspend new trades
- [x] `_exit_degraded_mode()`: set `state.degraded_mode = False`; log INFO; resume normal operation
- [x] Degraded trigger: 3 consecutive `tick_queue.get(timeout=60)` timeouts
- [x] Degraded exit: 3 ticks received within 5 seconds
- [x] Reconnect loop in degraded mode: exponential backoff (1,2,4,8,...,60s), max 10 attempts; after 10 → `sys.exit(2)`
- [x] Graceful shutdown: Ctrl+C → save state → log session summary → `sys.exit(0)`

### 7.4 **[NEW]** Latency Profiling (Verification)
- [x] Run `python tests/test_latency.py` with all 3 models loaded
- [x] Log p50, p95, p99 inference latencies
- [x] Assert p95 inference < 150ms (acceptable target)
- [x] If p95 > 150ms: document Mac hardware specs and proceed with WARNING (bricks are ~133 min — latency is not critical)

### 7.5 Integration Verification Gate
- [x] `BridgeEngine.start()` completes without error — "Bridge started" in log
- [x] After warmup: `brick_count >= 10`, `len(z_ofi.deque) >= 1000`, log shows "WARMUP DONE"
- [x] 30-minute dry run during market hours: ticks streaming, bricks forming, inference running
- [x] Verify `trades.csv` gets signal rows even before any trade is taken
- [x] First trade signal: verify `abs(sl_dist - brick_size) < 0.01` and `abs(tp_dist - brick_size) < 0.01`
- [x] SKIP signal: verify no order sent when `votes < 2`
...
- [x] Simulate crash: `kill -9 <pid>` mid-session with open position
- [x] Restart: `python bridge/main.py` → verify `state.json` loaded, active_ticket recognized
- [x] Assert: no duplicate trades opened; existing SL/TP untouched
