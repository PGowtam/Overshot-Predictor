"""
Offline Simulator for MTPATSC Trader
======================================
Simulates live trading by feeding ticks from a historical parquet file
into the MTPATSC BridgeEngine components. 
Fully replicates live market behavior including daily rollovers, 
path optimization for renko anchors, and real-time feature generation.
"""
...
def run_simulation(parquet_path: str):
    ...
    state = StateManager(filepath=str(BASE_DIR / "logs" / "sim_state.json"))
    state.reset()
    risk = RiskManager()
    
    # Clear old sim trades CSV to prevent stale data from prior runs
    sim_csv = str(BASE_DIR / "logs" / "sim_trades.csv")
    ...
    for i in range(len(bids)):
        ...
        for brick in new_bricks:
            ...
            if tensors is not None and active_ticket == 0:
                ...
                if result['action'] == 1:
                    ...
                    state.update("be_triggered", False)
        ...
        if active_ticket != 0:
            ...
            # Break-even check (disabled for T1)
            if not state.get("be_triggered"):
                setup_type_str = state.get("active_setup_type")
                if setup_type_str and setup_type_str != "T1":
                    if risk.check_be_trigger({'bid': bid, 'ask': ask}, state):
                        state.update("be_triggered", True)
                        # Offset SL by 0.5 pts to cover spread
                        new_sl = entry + 0.5 if direction == 1 else entry - 0.5
                        state.update("active_sl", new_sl)
            ...
            if direction == 1:  # BUY
                if bid <= sl:
                    outcome = "LOSS" if sl < entry else "BE"
                    exit_price = sl
                elif bid >= tp:
                    outcome = "WIN"
                    exit_price = tp
    ...
    logger_obj.generate_session_report(report_path=str(BASE_DIR / "logs" / "sim_session_report.md"))
    
    with open(BASE_DIR / "logs" / "sim_session_report.md", "r") as f:
        print("\n" + f.read())
