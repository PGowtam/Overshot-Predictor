...
OUTPUT_DIR = MTPATSC_DIR / "outputs" / "setup_classifier" / "dataset"
...
def process_chunk(year: int, month: int, files: list, target_start, target_end):
    ...
    log.info(f"Processing dataset chunk {year}-{month:02d}...")
    ...
    ptr = lib.generate_dataset(bids, asks, times, num_ticks, k_multiplier, ctypes.byref(out_num_rows))
    ...
    if out_num_rows.value > 0 and ptr:
        ...
        for i in range(out_num_rows.value):
            ...
            if target_start <= dt <= target_end:
                # Convert arrays and fill any accidental NaNs to 0
                ancs_f = np.nan_to_num(np.array(row.ancs_fine, dtype=np.float32).reshape(10, 6)).tolist()
                ancs_c = np.nan_to_num(np.array(row.ancs_coarse, dtype=np.float32).reshape(5, 6)).tolist()
                hist = np.nan_to_num(np.array(row.history, dtype=np.float32).reshape(5, 5, 6)).tolist()
                candle = np.nan_to_num(np.array(row.candle_features, dtype=np.float32)).tolist()
                mom = np.nan_to_num(np.array(row.momentum, dtype=np.float32)).tolist()
        ...
        if data:
            ...
            out_path = OUTPUT_DIR / f"dataset_{year}_{month:02d}.parquet"
            OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
            df_out.to_parquet(out_path)
            log.info(f"✅ Saved {len(df_out)} MTPATSC feature-labels to {out_path}")
            return len(df_out)
    return 0
...
def main():
    ...
    logger.info(f"Finished generating all unified datasets. Total valid setups: {total}")
