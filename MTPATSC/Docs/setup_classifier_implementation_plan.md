...
**Implementation**: C++ (`libfeature_engine_v4.dylib`) - *Heavy lifting moved from Python to C++ for performance.*
...
## 16. Advanced Quantitative Fixes

### 16.1 Label Hierarchy & Class Starvation
The concern is valid and will materialise. Here's the quantitative problem: T1 only gets a positive label when the market moves exactly 1K–2K (enough for T1 TP, not enough for T2 TP) OR when the T2 limit doesn't fill. Empirically this means T1 positive labels will be genuinely rare — not because T1 setups are rare, but because whenever T1 wins in a strong momentum market, T2 also wins and steals the label.
The fix is a two-stage approach. First, keep the priority hierarchy for the primary multi-class label — it reflects the best achievable RR on that brick and is the correct trading truth. Second, add a parallel auxiliary loss using the four raw binary labels:
```python
# During training, compute auxiliary BCE losses on the raw binary outcomes
loss_aux = (
    BCE(t1_win, prob_T1_raw) * 0.15 +
    BCE(t2_win, prob_T2_raw) * 0.15 +
    BCE(t3_win, prob_T3_raw) * 0.15 +
    BCE(t4_win, prob_T4_raw) * 0.15
)
loss_primary = CrossEntropy(label_5class, softmax_output) * 0.40
total_loss = loss_primary + loss_aux
```
This means T1's binary signal still teaches the network what a momentum-sustaining brick looks like, even when the multi-class label was assigned to T2. The 40/60 weighting keeps the primary 5-class objective dominant. Add four additional sigmoid output heads (one per binary label) that share the fused trunk but have their own single-neuron Dense output — remove them at inference time, keeping only the softmax head.

### 16.2 Limit Fill Assumption
The observation is correct and unfixable without L3 data. The right response is to quantify and document the optimism rather than try to correct for it. In the evaluation report, add a fill pessimism sensitivity analysis:
```python
# For T2 and T4 trades, apply a random fill rejection rate r
# and measure how EV degrades
for fill_rate in [1.0, 0.95, 0.90, 0.80, 0.70]:
    mask_filled = np.random.random(n_t2_trades) < fill_rate
    ev_adjusted = compute_ev(y_true[mask_filled], ...)
```
Sweep fill rates from 100% down to 70% and report the EV at each level. This tells you the system remains profitable even if 20–30% of limit orders are rejected at touch due to queue position. If T2 EV goes negative below 85% fill rate, that's a risk flag for live deployment. If it stays positive down to 70%, the strategy is robust. Document the break-even fill rate in evaluation_report.json so the live engine team knows the minimum acceptable fill quality.

### 16.3 Label Smoothing at Sharp Boundaries
The tension here is real: programmatic labels are exact but the market process generating them is continuous. The 0.1-pip-away-from-SL case is genuinely ambiguous economically even if it's unambiguous mathematically.
The correct approach is conditional label smoothing — apply smoothing only to samples where the outcome was marginal. Add a boundary_proximity flag to the label parquet:
```python
# In the label generator, record how "clean" the outcome was
boundary_proximity = abs(y_mag - 1.0)  # Distance from the TP/SL boundary
# boundary_proximity ≈ 0 means the trade barely won or barely lost
# boundary_proximity >> 0 means the outcome was unambiguous
```
Then during training, apply smoothing proportional to boundary proximity:
```python
# Instead of global label smoothing, smooth only near-boundary samples
epsilon = 0.05 * np.exp(-boundary_proximity * 10)
# Samples far from boundary: epsilon ≈ 0 (no smoothing)
# Samples at boundary: epsilon ≈ 0.05 (full smoothing)
smoothed_label = (1 - epsilon) * one_hot_label + epsilon / 5
```
This avoids over-smoothing clean wins and losses while regularising the model on the genuinely ambiguous boundary cases. Don't apply global smoothing as a first step — only reach for it if you observe val_accuracy plateauing while val_loss keeps decreasing, which is the signature of overconfidence on boundary samples.

### 16.4 Normalisation Before Fusion
The concern is precisely correct. log_duration can reach values of 8–10 for very long bricks, while proportion features are bounded in [0, 1] and tick count fraction is in [0, 0.05]. The Dense layers will see a 200× scale difference across features in the scalar input.
The cleanest solution is pre-normalisation of the scalar vector using a running stats normaliser fitted on the training set (not BatchNorm, for reasons explained below):
```python
# In sc_tensor_builder.py, after computing all scalar features on train set:
from sklearn.preprocessing import RobustScaler
# RobustScaler is better than StandardScaler here because 
# log_duration and path_complexity have outliers
scaler = RobustScaler()
X_train_scalars = scaler.fit_transform(X_train_scalars)
X_val_scalars = scaler.transform(X_val_scalars)
X_test_scalars = scaler.transform(X_test_scalars)

# Save the scaler for live inference
import joblib
joblib.dump(scaler, 'outputs/setup_classifier/scalar_scaler.pkl')
```
Why RobustScaler not StandardScaler: momentum features and log_duration will have occasional extreme outliers from high-volatility events. RobustScaler uses median and IQR instead of mean and std, making it resistant to those outliers without clipping them.
Why not BatchNormalization in the model: BatchNorm behaves differently during training vs inference (uses running statistics vs batch statistics), and in our small-batch financial data environment this creates a subtle train/test distribution mismatch. Pre-normalising the scalars before they enter the model at all is simpler, more transparent, and produces identical behaviour at training and inference time — which is critical when this model runs live tick-by-tick.
Save the RobustScaler as scalar_scaler.pkl alongside the model. The live inference engine loads both and applies the transform before every prediction call.
