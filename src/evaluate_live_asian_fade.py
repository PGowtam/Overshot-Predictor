...
# ── Constants ──────────────────────────────────────────────────────
EXEC_DIR = BASE_DIR / "outputs" / "exec_attn_regime"
MODEL_PATH = EXEC_DIR / "model.keras"
OUTPUT_DIR = BASE_DIR / "outputs" / "experiments" / "sim_asian_fade_2026"
...
def process_month(year, month, parquet_path):
    ...
    def predict_fast(micro, macro):
        return model([micro, macro], training=False)
        
    # Instantiate 6 independent brokers for Asian Fade
    brokers = {
        "0.45": SimulatedBroker("PW_0.45"),
        "0.40": SimulatedBroker("PW_0.40"),
        "0.35": SimulatedBroker("PW_0.35"),
        "0.30": SimulatedBroker("PW_0.30"),
        "0.25": SimulatedBroker("PW_0.25"),
    ...
    daily_summaries = {"0.45": [], "0.40": [], "0.35": [], "0.30": [], "0.25": [], "0.20": []}
        ...
        for tick in day_ticks:
            ...
            for brick in renko.update_tick(tick['bid'], tick['time_msc']):
                ...
                if tensors is not None and is_active_month:
                    micro, macro = tensors
                    preds = predict_fast(micro, macro)
                    ...
                    import datetime
                    dt = datetime.datetime.utcfromtimestamp(tick['time_msc'] / 1000.0)
                    hour = dt.hour
                    
                    if hour >= 20 or hour < 8:
                        thresholds = {"0.45": 0.45, "0.40": 0.40, "0.35": 0.35, "0.30": 0.30, "0.25": 0.25, "0.20": 0.20}
...
def main():
    ...
    print(f"🚀 Starting Sequential Simulation (Asian Fade Model)...")
    start_t = time.time()
    
    results = []
    for t in tasks:
        results.append(process_month(*t))
        
    all_trades = {"0.45": [], "0.40": [], "0.35": [], "0.30": [], "0.25": [], "0.20": []}
    all_summaries = {"0.45": [], "0.40": [], "0.35": [], "0.30": [], "0.25": [], "0.20": []}
