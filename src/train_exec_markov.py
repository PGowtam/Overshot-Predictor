...
from models_exec import build_markov_exec_model, compile_model

OUTPUT_DIR = BASE_DIR / "outputs" / "exec_markov"
...
def generate_markdown_report(history):
    ...
    report = f"""# Execution Markov Model (Model B) - Training Report

## Configuration
- **Model**: Triple-Input CNN+LSTM (Micro + Macro + Markov Sequence)
...
*(Model saved to outputs/exec_markov/model.keras)*
"""
...
def main():
    ...
    print("\n🏗️  Building Markov Execution Model...")
    model = build_markov_exec_model()
    ...
    history = model.fit(
        ...
        validation_data=(
            [data["val"]["micro"], data["val"]["macro"], data["val"]["seq"]],
