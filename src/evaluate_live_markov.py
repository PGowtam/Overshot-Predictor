...
class ExecPredictor:
    ...
    def predict(self, micro_tensor, macro_tensor, seq_tensor):
        """Run inference. Returns (action, prob_win, pred_os)."""
        if self.model is None:
            return 0, 0.0, 0.0
        
        preds = self.model([micro_tensor, macro_tensor, seq_tensor], training=False)
