...
OUTPUT_DIR = BASE_DIR / "outputs" / "experiments" / "sim_2026"
...
class FallbackPredictor:
    """Loads the fallback model and applies calibrated thresholds."""
    
    def __init__(self):
        self.model = None
        self.prob_win_threshold = 0.52   # EXP-11 optimal
        self.pred_os_threshold = 1.0     # EXP-11 optimal
        self.T = 1.0                      # Temperature scaling
    
    def load(self):
        logger.info(f"Loading fallback model from {MODEL_PATH}...")
        self.model = tf.keras.models.load_model(str(MODEL_PATH), compile=False)
        
        # Load EXP-11 calibration
        calib_path = EXP11_DIR / "calibration_params.json"
        if calib_path.exists():
            with open(calib_path, "r") as f:
                calib = json.load(f)
                self.T = calib.get("T", 1.0)
            logger.info(f"Loaded calibration: T={self.T:.4f}")
        ...
        logger.info(f"Using exact fallback thresholds: PW>={self.prob_win_threshold}, OS>={self.pred_os_threshold}")
