...
# ── Constants ──────────────────────────────────────────────────────
ATTN_DIR = BASE_DIR / "outputs" / "exec_attn_regime"
...
class AsianFadePredictor:
    """Loads the Attention Regime model and executes Asian Fade."""
    
    def __init__(self):
        self.model = None
        self.predict_fast = None
        self.prob_win_threshold = 0.45   # Our optimal Asian Fade threshold
    
    def load(self):
        logger.info(f"Loading Attention Regime model from {MODEL_PATH}...")
        
        sys.path.insert(0, str(BASE_DIR / "src"))
        from models_exec import build_attention_exec_model, HIGH_MI_DIMS
        self.model = build_attention_exec_model(high_mi_dims=HIGH_MI_DIMS)
        self.model.load_weights(str(MODEL_PATH))
        logger.info(f"Using exact Asian Fade threshold: PW <= {self.prob_win_threshold}")
    ...
    def predict(self, micro_tensor, macro_tensor, tick_time_msc):
        ...
        # Check Asian Session (20:00 to 08:00 UTC)
        import datetime
        dt = datetime.datetime.utcfromtimestamp(tick_time_msc / 1000.0)
        hour = dt.hour
        is_asian = hour >= 20 or hour < 8
        
        # Action = 1 means FADE
        signal = is_asian and (pw <= self.prob_win_threshold)
...
class SimEngine:
    ...
    def __init__(self, tick_provider: TickReplayProvider):
        ...
        self.predictor = AsianFadePredictor()
